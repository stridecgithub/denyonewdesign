...
export class LoginPage {
  ...
  ionViewDidLoad() {
    ...
    var simpsonAutocompleter = jQuery.MentionsKinder.Autocompleter.Select2Autocompleter.extend({
      select2Options: {
          data: [
  ...
  var tagAutocompleter = jQuery.MentionsKinder.Autocompleter.Select2Autocompleter.extend({
      select2Options: {
          tags: ["red", "green", "blue"]
      }
  });
  ...
  jQuery.MentionsKinder.defaultOptions.trigger['@'].autocompleter = simpsonAutocompleter;

  // Init our simplest input by using defaults
  jQuery('.simple').mentionsKinder().enableMyDebug();
  // Init customized input with tag autocompleter
  jQuery('.tags').mentionsKinder({
      trigger: {
          '#': {
              triggerName: 'tag',
  ...
  jQuery('.multiple').mentionsKinder({
      trigger: {
          '#': {
              triggerName: 'tag',
              autocompleter: tagAutocompleter
  ...
  // bind "change" event
  jQuery('#event_test').on('change', function(event) { jQuery('#event_timestamp').text(new Date) });


  jQuery(document.body).on('click', '[data-action=set-flag]', function(e){
      var $button = jQuery(e.currentTarget),
              $target = jQuery($button.data('target')),
              flag = $button.text(),
              regex = /(?:^|\s+)!(\w+)/i,
              val = $target.val();
